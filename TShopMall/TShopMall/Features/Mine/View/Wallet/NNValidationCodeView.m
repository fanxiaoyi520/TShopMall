...
- (instancetype)initWithFrame:(CGRect)frame andLabelCount:(NSInteger)labelCount andLabelDistance:(CGFloat)labelDistance {
    ...
    if (self) {
        ...
        self.defaultColor = KHexColor(@"#D4D4D4");
        [self setupViews];
    }
    return self;
}
...
- (void)setupViews {
    ...
    for (int i = 0; i < self.labelCount; i++) {
        ...
        label.backgroundColor = KHexColor(@"#F4F4F4");
        [label jaf_customFilletRectCorner:UIRectCornerAllCorners cornerRadii:CGSizeMake(4, 4)];
...
- (void)textFieldDidChange:(UITextField *)textField {
    ...
    if (i == 0) {
        ...
        ((UILabel *)[self.labelArr objectAtIndex:i - 1]).text = @"*";
...
- (void)clearPassword {
    [self.labelArr enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (self.labelCount > idx) {
            ((UILabel *)[self.labelArr objectAtIndex:idx]).text = @"";
            ((UILabel *)[self.labelArr objectAtIndex:idx]).layer.borderColor = _defaultColor.CGColor;
        }
    }];
}
