...
- (instancetype)init {
    if (self = [super init]) {
        ...
        self.interitemSpacing = KRateW(8.0);
        self.lineSpacing = KRateW(16.0);
        self.itemHeight = 30.0;
        self.itemInset = UIEdgeInsetsZero;
        self.headerInset = UIEdgeInsetsZero;
        self.footerInset = UIEdgeInsetsZero;
        self.labelFont = KRegularFont(14.0);
        self.sectionInset = UIEdgeInsetsMake(0, KRateW(16.0), 0, KRateW(16.0));
...
- (void)prepareLayout {
    ...
    self.viewWidth = kScreenWidth - self.itemInset.left - self.itemInset.right;
    //所有内容的布局属性数组
...
- (CGFloat)countItemSizeWithIndexPath:(NSIndexPath *)indexPath {
    ...
    return MAX(size.width + 20.0, self.itemHeight);
}
