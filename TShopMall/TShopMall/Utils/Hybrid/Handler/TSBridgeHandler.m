...
//        vc.toltalDiscount = [NSString stringWithFormat:@"%.2f",discountMoney];
    ...
    //    vc.fullReductionAmount=weakSelf.fullTotalAmount;
        ...
        if (([TSAddressManager shareManager].defaultModel.provinceName.length <= 0)) {//没有默认地址
            UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"温馨提示"
                                                                                     message:@"您还没设置默认地址，请前往设置"
            ...
            UIAlertAction *okAction = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                ...
                NSString *address;
                if ([TSUserInfoManager shareManager].userCurrentInfo.isGreatB || [TSUserInfoManager shareManager].userCurrentInfo.isDirectSales) {
                    address = kMineAddress;//代理商收货地址
                } else {
                    address = kMineAgentAddress;//被代理商收货地址
                    
                }
                NSString *url = [NSString stringWithFormat:@"%@%@",kH5Prefix,address];
                TSHybridViewController *hybrid = [[TSHybridViewController alloc] initWithURLString:url];
                
                [controller.navigationController pushViewController:hybrid animated:YES];
