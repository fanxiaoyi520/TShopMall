...
- (void)setModel:(BasicDialogModel *)model {
    ...
    if (textModel.attributedTextContent.string.length > 0 || textModel.textContent.length > 0) {
        ...
        CGFloat textHeight = [DialogTextTool richTextHeightWithAttributedString:self.textLabel.attributedText width:W];
...
+ (CGFloat)elementHeightWithModel:(BasicDialogModel *)model elementWidth:(CGFloat)elementWidth {
    ...
    CGFloat height = 0;
    if (textModel.attributedTextContent.string.length > 0) {
        height = [DialogTextTool richTextHeightWithAttributedString:textModel.attributedTextContent width:width] + textModel.textEdgeInsets.top +
            textModel.textEdgeInsets.bottom;
    } else if (textModel.textContent.length > 0) {
        NSString *textColor = textModel.textColor.length > 0 ? textModel.textColor : DefaultTextColor;
        CGFloat fontSize = textModel.fontSize > 0 ? textModel.fontSize : DefaultFontSize;
        CGFloat lineSpace = textModel.lineSpace > 0 ? textModel.lineSpace : DefaultLineSpace;

        NSAttributedString *attributedText = [DialogTextTool createAttributedStringWithTextContent:textModel.textContent textAlignment:textModel.textAlignment
                                                                                          FontSize:fontSize
                                                                                         lineSpace:lineSpace
                                                                                         textColor:[UIColor colorWithHexString:textColor]
                                                                                     richTextArray:textModel.richText
                                                                                         breakMode:0];
        height =
            [DialogTextTool richTextHeightWithAttributedString:attributedText width:width] + textModel.textEdgeInsets.top + textModel.textEdgeInsets.bottom;
    } else {
