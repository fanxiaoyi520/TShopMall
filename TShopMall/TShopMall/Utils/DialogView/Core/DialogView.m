...
static const CGFloat cornerRadius = 8.0;                              //圆角
...
- (void)setUIDetailUI {
    ...
    if (self.customView) {
        ...
        for (NSInteger i = 0; i < self.dialogModels.count; i++) {
            ...
            CGFloat H = [NSClassFromString(model.elementClassName) elementHeightWithModel:model elementWidth:W];
    ...
    self.finalRect = CGRectMake((DIALOG_SCREEN_WIDTH - self.contentViewWidth) / 2, finalRectY, self.contentViewWidth, self.contentViewHeight);
...
- (void)unholdInQueue {
    ...
    if (self.unholdInQueueBlock) {
        self.unholdInQueueBlock();
    }
}
...
- (void)closeDialogViewWithAnimation:(BOOL)animation holdInQueue:(BOOL)holdInQueue {
    ...
    [UIView animateWithDuration:time animations:^{
        ...
        if (self.unholdInQueueBlock && !holdInQueue) {
            self.unholdInQueueBlock();
        }
...
- (void)keyboardShowAction:(NSNotification *)sender {
    ...
    if (CGRectGetMaxY(self.finalRect) > (DIALOG_SCREEN_HEIGHT - keyBoardHeight)) {
        [UIView animateWithDuration:0.2 animations:^{
            ...
            newRect.origin.y = DIALOG_SCREEN_HEIGHT - keyBoardHeight - CGRectGetHeight(self.finalRect) - 24.0;
