...
+ (NSMutableAttributedString *)createAttributedStringWithTextContent:(NSString *)textContent
    ...
    [attributedText addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:fontSize] range:NSMakeRange(0, textContent.length)];
        ...
        if (richType.length > 0 && rangeArray.count == 2) {
            ...
            if ([richType isEqualToString:@"color"]) {
                ...
                if (textColorString.length > 0) {
                    UIColor *color = [UIColor colorWithHexString:textColorString];
                ...
                if (richDic[@"fontSize"]) {
                    ...
                    UIFont *font =
                        [richDic[@"bold"] isEqualToString:@"true"] ? [UIFont boldSystemFontOfSize:fontSizeFloat] : [UIFont systemFontOfSize:fontSizeFloat];
...
/// @param width 固定宽度
+ (CGFloat)richTextHeightWithAttributedString:(NSAttributedString *)attributedString width:(CGFloat)width {
